...
def read_gz(files):
    return TFRecordDataset(files, compression_type="gzip")
...
def get_serve_fn(model, transform_graph):
    ...
    def serve_fn(serialized_examples):
        ...
        transformed_features = transform_graph.transform_features_layer(parsed_features)
...
def run_fn(args):
    transform_graph = TFTransformOutput(args.transform_output_dir)
    train_dataset = input_fn(args.train_files, transform_graph)
    val_dataset = input_fn(args.val_files, transform_graph)
